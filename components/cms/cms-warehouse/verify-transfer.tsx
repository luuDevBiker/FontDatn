...
export const VerifyTransfer:NextPageWithLayout=()=>{
    ...
    useEffect(()=>{
        ...
        dispatch(getDeliveryById(payload)).unwrap().then().then((res:any)=>{
            setData(res)
            console.log(data)
        })
    },[])
    ...
    [
        {
          ...
          render: (text: string, record: any) =>(
            <a onClick={()=>router.push(`/cms/cms-warehouse/create-tranfers/${record.recivedBillId}/`)}>{text}</a>
          )
        },
        {
            title: 'SKU',
        ...
        },
        {
            ...
            render:(record:IResponseRecivedBill)=>(
                <Space>
                    <InputNumber max={sumOfProduct} min={0} type="number" onChange={onChangeImport}/>
                    <Button>All</Button>
                </Space>
            )
        },
        {
            ...
            render:(record:IResponseRecivedBill)=>(
                <Space>
                    <InputNumber max={sumOfProduct} min={0} type="number" onChange={onChangeReject}/>
                    <Button>All</Button>
                </Space>
            )
        }
    ...
    return (
        <WrapperCMSProduct>
            <WrapProduct>
                ...
                <div>
                    <Progress {...config} />
                    <div>Tổng số nhận được: {numImport}/{sumOfProduct}</div>
                    <br/>
                </div>
                <div>
