...
export const CreateEmployees:NextPageWithLayout=()=>{
    ...
    const handleSubmit=(values:any)=>{
        ...
        dispatch(createEmployees(payload)).unwrap().then().then((res:any)=>{
            message.success({
    ...
    const onFieldsChange = (changedFields:any, allValues:any) => {
        if (changedFields && changedFields[0].name[0] === 'City') {
          console.log("chane",changedFields[0])
          ...
          dispatch(getConfigurations(payload)).unwrap().then().then((res:any)=>{
            if(res){
                        ...
                        <WrapProduct>
                                ...
                                >
                                    <Dragger {...props}>
                                        ...
                                        <p className="ant-upload-text">Nhấn vào hoặc kéo thả ảnh bạn cần upload</p>
                                    </Dragger>
                                </Form.Item>
                                  ...
                                  <Row gutter={[16, 16]}>
                                    <Col xxl={8} xl={8} sm={12} xs={24}>
                                      ...
                                      <Form.Item name={'District'} label='Quận/Huyện'>
                                          <Select  showSearch allowClear placeholder={'Quận/Huyện'} disabled={checkDistrict}
                                          dropdownStyle={{ minWidth: "20%", height: "100px" }}
                                          filterOption={(input:string, option:any) =>  
                                          option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0 
                                          || option.props.value.toLowerCase().indexOf(input.toLowerCase()) >= 0
                                      ...
                                      <Form.Item name={'Ward'} label='Phường/Xã'>
                                          <Select showSearch allowClear placeholder={'Phường/Xã'}
                                          dropdownStyle={{ minWidth: "20%", height: "100px" }} disabled={checkWard}
                                          filterOption={(input:string, option:any) =>  
                                          option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0 
                                          || option.props.value.toLowerCase().indexOf(input.toLowerCase()) >= 0
                                          }
